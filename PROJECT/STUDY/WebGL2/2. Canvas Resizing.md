모든 캔버스는 **2개의 크기**를 가짐 
1. drawingbuffer
2. canvas의 diplay size(화면에 표시되는 크기. CSS가 화면에 표시되는 크기를 결정함)

캔버스의 drawingBuffer 크기를 정하는 방법이 두 가지 있음. 

```html title:"1. HTML을 사용한  drawingBuffer 크기 설정"
...
> [!NOTE]
>  JavaScript에서는 *canvas*가 화면에 출력되는 크기는 영향을 미치는 ***CSS***가 없다면 **drawingbuffer**와 동일한 사이즈로 출력 
>  즉, 400x300 이므로 화면에 표시되는 크기도 400x300
...
<html> <head> <script> window.onload = function() { var iframe = document.getElementById("myiframe"); iframe.width = iframe.contentWindow.document.body.scrollWidth; iframe.height = iframe.contentWindow.document.body.scrollHeight; } </script> </head> <body> <iframe id="myiframe" src="https://webgl2fundamentals.org/webgl/webgl-10x15-canvas-400x300-css.html"></iframe> </body> </html>

![[Excalidraw/Drawing 2024-11-05 13.48.58.excalidraw.md#^area=A99D_-Lw5I7zQMN4f_f9p|center]]


10x15 픽셀크기의 캔버스를 400x300 크기로 늘렸기 때문

### 캔버스가 윈도우 전체 영역이 되도록 늘리기 위해서는? 

CSS를 사용해 캔버스가 윈도우 전체 영역이 되도록 한다. 그 후, drawingBuffer의 크기를 브라우저가 늘려놓은 캔버스의 크기와 일치시키기만 하면 된다. 불행하게도 이는 복잡한 문제이다. 
...
#### 가장 쉬운 방법!
- `clientWidth`와 `clientHeight`는 HTML의 모든 요소(element)가 가진 속성으로, CSS pixel 단위로 element의 크기를 알려줌.

> [!NOTE] Warnning
> clientRect는 CSS 패딩을 포함하므로 `clientWidth`와 `clientHeight`를 사용할 때는 캔버스 요소에 *padding*을 사용하지 않는 것이 좋습니다.
...
**결과**:  1차 개선만으로는 화면에 선이 꽉 채워 지지 않음. **캔버스는 커짐**


이유는 캔버스의 사이즈를 수정하기 전, `gl.viewport`를 호출하여 뷰포트를 설정해야 하기  때문.

`gl.viewport` : WebGL에게 클립 공간(-1 ~ 1)에서 픽셀로 변환하는 법과 그러한 변환이 캔버스의 어떤 부분에서 이뤄져야 하는지 알려줌. context 생성시, WebGL은 `viewport`를 캔버스의 크기와 동일 하도록 설정해줌. 하지만 그 이후엔 사용자가 설정해야 함. **만일 캔버스의 크기를 바꿨다면, WebGL에게 viewport 설정을 알려줘야 한다.**
