...
Vertex Shader의 역할은 정점의 위치를 계산하는것. 함수를 통해 출력 위치가 계산되고, WebGL은 그 위치를 기준으로 'Point', 'Line', 'Triangle'을 비롯한 다양한 종류의 기본요소(Primitives)를 **래스터화** 할 수 있다. 
...
속성 상태, 각 속성에 사용할 버퍼, 해당 버퍼에서 데이터를 가져오는 방법은 [[VAO (Vertex Array Object)]] 에 수집 됨. 
