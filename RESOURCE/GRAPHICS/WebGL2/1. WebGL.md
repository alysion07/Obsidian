...
전형적인 WebGL 프로그램은 보통 아래와 같은 구조를 따릅니다.
##### At Initial-time
- create ***shader***, and create ***program***, look up ***location***(uniform, attribute)
- create ***buffer*** and upload ***vertex data*** 
- 그리고자 하는 것에 대해 ***vertex array*** 생성
	- 각 ***attribute*** 마다`gl.bindBuffer`, `gl.vertexAttribPointer`, `gl.enableVertexAttribArray` 호출
	- 인덱스 관련 데이터는 `gl.ELEMENT_ARRAY_BUFFER`에 바인딩
- create ***textures*** and upload ***texture data***
##### At Render Time
- clear and set ***viewport*** and 나머지 ***global state***(
...
- 그리고자 하는 물체마다,
    ...
    - 그리고자 하는 물체의 uniform 설정
        - 각 ***uniform***마다 `gl.uniformXXX` 호출
        - ***texture***를 texture unit에 할당하기 위해 `gl.activeTexture`와 `gl.bindTexture` 호출
...
**uniform value**:  Shader 프로그램을 실행하기 전에 설정하는 전역 변수
