...
```js title:'JS : get Rotation value from UI'
...
```cs title:'Shader Code - get rotated 'x', 'y' value'
rotatedX = a_position.x * u_rotation.y + a_position.y * u_rotation.x;
rotatedY = a_position.y * u_rotation.y - a_position.x * u_rotation.x;
```

참고 노트: [[ClipSpace 좌표계 변환]]
...
```js title:'Geometry Rotation'  hl:47,51-54,145-151,102,129,172
